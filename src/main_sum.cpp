...
void run(int argc, char** argv)
{
    ...
    // Прогружаем входные данные по PCI-E шине: CPU RAM -> GPU VRAM
    {
        ...
        std::cout << "load times (in seconds) - " << stats::valuesStatsLine(times) << std::endl;

        size_t array_size = values.size() * sizeof(unsigned int);
        double bw = static_cast<double>(array_size) / stats::median(times);
        std::cout << "PCI-E Bandwidth: " << bw / (1024 * 1024 * 1024) << " GiB/s" << std::endl;
        std::cout << "PCI-E Bandwidth: " << bw / (1000 * 1000 * 1000) << " GB/s" << std::endl;
        // PCI-E Gen 4 x8 SoL: 15.754
        // Observed via nvtop: ~13
        // Output: PCI-E Bandwidth: 12.0425 GB/s
        // В пределах "погрешности" GiB vs GB
    }
            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeOpenCL) {
                    ...
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        size_t reduction_size = div_ceil(n, (unsigned int) GROUP_SIZE);
                        while (reduction_size > 1) {
                            cuda::sum_04_local_reduction(
                                gpu::WorkSize(GROUP_SIZE, reduction_size),
                                reduction_buffer1_gpu,
                                reduction_buffer2_gpu,
                                reduction_size
                            );
                            reduction_size = div_ceil(reduction_size, (size_t)GROUP_SIZE);
